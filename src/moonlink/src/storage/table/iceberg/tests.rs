...
/// B-5c regression: every persisted snapshot lays Hudi-pattern markers under
/// `<private_root>/<table_uuid>/.markers/<snapshot_id>/` before
/// `PrivateManifestStore::write_snap_manifest` runs. The set of marker `file_path`
/// payloads must cover both the snap-`<id>`.json manifest URI and every newly
/// added hash-puffin file in this commit.
///
/// Markers persist until B-5d wires `commit_success`, so this test reads them
/// directly off the filesystem. When B-5d lands, this assertion flips to "marker
/// dir is empty after commit"; the path-coverage check moves into the B-5d
/// "rollback after write_snap_manifest failure" test instead.
#[tokio::test]
async fn test_b5c_pre_write_markers_cover_new_artifacts() {
    ...
    // Locate the marker directory off the local filesystem (the test uses a
    // FileSystem-backed catalog rooted at iceberg_temp_dir). There is exactly one
    // table_uuid subdir and one snapshot id beneath `.markers/`, so both are
    // discovered by walking the on-disk layout.
    ...
    let snap_dirs: Vec<_> = std::fs::read_dir(&markers_root)
        .unwrap()
        .filter_map(|e| e.ok())
        .collect();
    assert_eq!(snap_dirs.len(), 1, "expected one snapshot markers dir");
    let snap_dir = snap_dirs[0].path();
    let snap_id_str = snap_dir.file_name().unwrap().to_string_lossy().to_string();

    let marker_files: Vec<_> = std::fs::read_dir(&snap_dir)
        .unwrap()
        .filter_map(|e| e.ok())
        .collect();
    assert!(!marker_files.is_empty(), "no markers laid for snapshot");

    // Each marker payload's `file_path` must point at an artifact under the table
    // root. The manifest URI must be present; puffin URIs (one per new file index)
    // come along when a hash blob is generated by this commit.
    let mut recorded_paths = StdHashSet::<String>::new();
    for entry in &marker_files {
        ...
        let path = value
            .get("file_path")
            .and_then(|v| v.as_str())
            .expect("marker missing file_path")
            .to_string();
        recorded_paths.insert(path);
    }
    let expected_manifest_path = format!(
        "{}/manifest/snap-{}.json",
        table_root.to_string_lossy(),
        snap_id_str
    );
    assert!(
        recorded_paths.contains(&expected_manifest_path),
        "manifest path missing from markers: have {recorded_paths:?}, want {expected_manifest_path}"
    );

    // Stronger: every hash-puffin file referenced by the persisted manifest must
    // also have a marker. Without this, dropping the `for puffin_path in
    // &newly_added_puffin_paths` loop in `persist_private_manifest` would still
    // pass the previous assertion. This is the first commit in the test, so the
    // manifest has no inherited parent entries — every entry counts as new.
    let manifest_bytes = std::fs::read(&expected_manifest_path).unwrap();
    ...
    for entry in entries {
        let puffin = entry
            .get("puffin_file_path")
            .and_then(|v| v.as_str())
            .expect("manifest entry missing puffin_file_path");
        assert!(
            recorded_paths.contains(puffin),
            "puffin {puffin} present in manifest but missing from markers: {recorded_paths:?}"
        );
    }
}
